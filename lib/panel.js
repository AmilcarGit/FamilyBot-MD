...
const OWNER_PRINCIPAL = '51910227479'
...
function obtenerToken() {
  ...
  if (!esTokenDefault) {
    tokenUnico = tokenActual
    return tokenUnico
  }

  tokenUnico = 'yui-' + randomBytes(4).toString('hex')
  config.panelToken = tokenUnico
    ...
    content = `
      <div class="card" style="max-width:400px; margin:0 auto;">
          ...
          <form action="/action/config" method="POST">
              <input type="hidden" name="token" value="${t}">
              <label style="color:#00d4ff">Identidad del Bot (Fija en config.js)</label>
              <input value="${config.nombreBot}" disabled style="opacity:0.6; border-color:#00d4ff">
              <label>Prefijo</label><input name="prefijo" value="${config.prefijo}">
              <label style="color:#ff007f">Owner Principal (Creador)</label>
              <input value="${OWNER_PRINCIPAL}" disabled style="opacity:0.6; border-color:#ff007f">
<label>Owner Secundario</label>
                  <input name="ownerSecundario" value="${config.owner[1] || ''}" placeholder="Ej: 5493875132593">
                  <label>Prioridad de Respuesta (0 = Principal)</label>
                  <input type="number" name="prioridad" value="${config.prioridad || 0}" min="0" max="10">
                  <h4 style="margin:20px 0 10px 0; color:#ff007f; font-size:12px;">Gestión de Staff</h4>
              <div id="staff-fields">
                  ${(config.staff || []).map((s, idx) => `
                    <div style="display:flex; gap:5px; margin-bottom:5px;">
                        <input name="staff_n_${idx}" value="${s.nombre}" placeholder="Nombre" style="flex:1; margin:0">
                        <input name="staff_u_${idx}" value="${s.numero}" placeholder="Número" style="flex:1; margin:0">
                    </div>
                  `).join('')}
                  <div style="display:flex; gap:5px; margin-bottom:5px;">
                      <input name="staff_n_new" placeholder="Nuevo Staff Nombre" style="flex:1; margin:0">
                      <input name="staff_u_new" placeholder="Nuevo Staff Número" style="flex:1; margin:0">
                  </div>
              </div>
              <button type="submit" class="btn" style="margin-top:15px;">Guardar Cambios</button>
          </form>
      </div>
    `
  } else if (tab === 'cmds') {
    ...
    <script>
        ...
        async function sync() {
            try {
                ...
                var box = document.querySelector('.term');
                if(box) {
...
export function iniciarPanel() {
  ...
  app.post('/action/config', (req, res) => {
    const { token, prefijo, ownerSecundario, prioridad } = req.body
    if (token !== obtenerToken()) return res.status(401).send('Error')
    
    const nuevosOwners = [OWNER_PRINCIPAL]
    if (ownerSecundario && ownerSecundario.trim()) {
      nuevosOwners.push(ownerSecundario.trim().replace(/\D/g, ''))
    }

    const nuevoStaff = []
    Object.keys(req.body).forEach(key => {
      if (key.startsWith('staff_n_')) {
        const idx = key.replace('staff_n_', '')
        const nombre = req.body[key]
        const numero = req.body[`staff_u_${idx}`]
        if (nombre && numero) {
          nuevoStaff.push({ nombre, numero: numero.replace(/\D/g, '') })
        }
      }
    })

    guardarConfig({ ...config, prefijo, prioridad: parseInt(prioridad, 10) || 0, owner: nuevosOwners, staff: nuevoStaff })
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.log('\n' + '═'.repeat(50))
      console.log('⚠️  PUERTO ' + config.panelPort + ' OCUPADO. REINTENTANDO LIMPIEZA...')
      console.log('═'.repeat(50) + '\n')
