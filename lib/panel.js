...
// obtenerComandosPanel se importa de forma diferida (ver más abajo) para evitar
// una importación circular con handler.js, que a su vez carga commands/owner/panel.js
...
function renderPagina(token, tab = 'dash', extra = {}, soloFragmento = false) {
  const t = esc(token)
  const tabs = [
...
<head>
    ...
    <title>FamilyBot-MD · Panel</title>
    <style>
        :root {
        ...
        * { box-sizing: border-box; }
        ...
        .reproductor {
            ...
            background: rgba(13,13,22,.9); backdrop-filter: blur(6px);
            border: 1px solid var(--pink); border-radius: 999px;
            padding: 8px 14px; box-shadow: 0 0 20px rgba(255,45,149,.25);
        }
        .reproductor-btn {
            background: none; border: none; color: var(--text); font-size: 16px;
            cursor: pointer; padding: 2px 4px; border-radius: 50%;
        }
        .reproductor-btn:hover { background: rgba(255,255,255,.1); }
        .reproductor-info { font-size: 11px; color: var(--muted); max-width: 110px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    </style>
</head>
<body>
    <div class="fondo-decorativo" id="fondoDecorativo"></div>
    <div class="topbar">
        <div class="brand"><span>🦋</span><h2>FAMILYBOT-MD · PANEL</h2></div>
        <form action="/action/restart" method="POST" style="margin:0">
            <input type="hidden" name="token" value="${t}">
            <button type="submit" class="btn-s btn-red">REINICIAR</button>
        </form>
    </div>
    <div id="appContent" data-tab="${esc(tab)}">
        ${menu}
        ${content}
    </div>

    <div class="reproductor" id="reproductorMusica">
        <button class="reproductor-btn" id="btnPlayPausa" title="Reproducir/Pausar">▶️</button>
        <div class="reproductor-info">
            <span id="reproductorTitulo">🎵 Canción 1</span>
        </div>
        <button class="reproductor-btn reproductor-siguiente" id="btnSiguiente" title="Siguiente canción">⏭️</button>
        <audio id="audioPanel" preload="none">
            <source id="fuenteAudio" src="/music/cancion1.mp3?token=${t}" type="audio/mpeg">
        </audio>
    </div>
    <script>
        var tk = "${t}";
        ...
        async function cargarTab(url, guardarHistorial) {
            try {
                var separador = url.includes('?') ? '&' : '?';
                var resp = await fetch(url + separador + 'fragmento=1');
                if (!resp.ok) { window.location.href = url; return; }
                var html = await resp.text();
                appContent.innerHTML = html;
                if (guardarHistorial) history.pushState({ url: url }, '', url);
            } catch (e) {
        ...
        appContent.addEventListener('click', function (e) {
            ...
            if (!link) return;
            e.preventDefault();
            cargarTab(link.getAttribute('href'), true);
        });

        window.addEventListener('popstate', function (e) {
            cargarTab(window.location.pathname + window.location.search, false);
        });

        var pistas = [
            { archivo: 'cancion1.mp3', titulo: '🎵 Canción 1' },
            { archivo: 'cancion2.mp3', titulo: '🎵 Canción 2' },
        ];
        var pistaActual = 0;
        var audioPanel = document.getElementById('audioPanel');
        var btnPlayPausa = document.getElementById('btnPlayPausa');
        var btnSiguiente = document.getElementById('btnSiguiente');
        var reproductorTitulo = document.getElementById('reproductorTitulo');

        function cargarPista(indice, autoplay) {
            pistaActual = indice;
            var pista = pistas[pistaActual];
            audioPanel.src = '/music/' + pista.archivo + '?token=' + encodeURIComponent(tk);
            reproductorTitulo.textContent = pista.titulo;
            if (autoplay) audioPanel.play().catch(function () {});
        }

        if (btnPlayPausa) {
            btnPlayPausa.addEventListener('click', function () {
                if (!audioPanel.src) cargarPista(0, false);
                if (audioPanel.paused) {
                    audioPanel.play().catch(function () {
                        reproductorTitulo.textContent = '⚠️ Falta el archivo de audio';
                    });
        ...
        if (btnSiguiente) {
            btnSiguiente.addEventListener('click', function () {
                cargarPista((pistaActual + 1) % pistas.length, !audioPanel.paused);
                btnPlayPausa.textContent = '⏸️';
            });
        }
        ...
        if (audioPanel) {
            audioPanel.addEventListener('ended', function () {
                cargarPista((pistaActual + 1) % pistas.length, true);
            });
        }
...
export function iniciarPanel() {
  ...
  async function datosParaTab(tab) {
    if (tab === 'users') {
      const db = await getDB()
      return { tab: 'users', extra: { users: db.data.users } }
    }
    ...
    if (tab === 'alerts') {
      return {
        tab: 'alerts',
        extra: { alertas: alertsBuffer, badMacCount: contadorBadMac, ultimoBadMac },
      }
    }
    ...
    if (tab === 'dash') {
      ...
      return { tab: 'dash', extra: { stats: { grupos, usuarios } } }
    }

    if (tab === 'cmds') {
      const { obtenerComandosPanel } = await import('../handler.js')
      return { tab: 'cmds', extra: { cmds: obtenerComandosPanel() } }
    }
  ...
  app.get('/', async (req, res) => {
    ...
    const tabPedido = req.query.tab || 'dash'
    const realToken = obtenerToken()
    if (!token || !tokensCoinciden(token, realToken)) {
      return res.send(paginaLogin())
    }

    const { tab, extra } = await datosParaTab(tabPedido)
    const soloFragmento = req.query.fragmento === '1'
    res.send(renderPagina(token, tab, extra, soloFragmento))
  })
  ...
  app.get('/music/:archivo', (req, res) => {
    const token = req.query.token
    if (!tokensCoinciden(token, obtenerToken())) return res.status(401).send('Error')
    const nombre = path.basename(req.params.archivo)
    const ruta = path.join(process.cwd(), 'music', nombre)
...
function paginaLogin(mensajeError = '') {
  return `
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>FamilyBot-MD · Login</title>
        <style>
