...
function renderPagina(token, tab = 'dash', extra = {}) {
  const t = esc(token)
  const tabs = [
    ['dash', 'Inicio'], ['stats', 'Estadísticas'], ['users', 'Usuarios'],
    ...
    content = `
      <div class="card card-narrow">
          ...
          <form action="/action/config" method="POST">
              ...
              <label class="label-pink">Owner Principal (Creador)</label>
              <input value="${esc(OWNER_PRINCIPAL)}" disabled class="input-locked-pink">
              <label>Owner Secundario</label>
              <input name="ownerSecundario" value="${esc(config.owner[1] || '')}" placeholder="Ej: 5493875132593">
              ...
              <div id="staff-fields">
                  ...
                  `).join('')}
                  <div class="staff-row">
                      <input name="staff_n_new" placeholder="Nuevo staff — nombre">
                      <input name="staff_u_new" placeholder="Nuevo staff — número">
                  </div>
              </div>
