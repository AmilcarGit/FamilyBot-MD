...
<script>
  ...
  async function cargarSubbots() {
    ...
    cuerpo.innerHTML = data.length
      ...
      : '<tr><td colspan="3" class="vacio">No hay subbots activos en este momento</td></tr>'
  }
  async function cargarGrupos() {
    ...
    cuerpo.innerHTML = data.length
      ...
      : '<tr><td colspan="2" class="vacio">Sin información de grupos disponible</td></tr>'
  }
  async function cargarEconomia() {
    ...
    cuerpo.innerHTML = data.length
      ? data.map((u, i) => \`<tr><td>\${i + 1}</td><td>\${u.numero}</td><td>\${u.total.toLocaleString()} 💰</td></tr>\`).join('')
      : '<tr><td colspan="3" class="vacio">Todavía no hay datos económicos registrados</td></tr>'
  }
  async function cargarLogs() {
    const res = await fetch('/api/logs', { headers: cabecera() })
    const data = await res.json()
    const caja = document.getElementById('cajaLogs')
    caja.textContent = data.texto || 'Esperando registros de la terminal...'
